...
async function verifyBBSProof(proof, publicKey, revealedMessages, nonce) {
  console.log("Verifying proof...");
    ...
    //const nonceToVerify = Uint8Array.from(Buffer.from(nonce, "utf8"));
     ...
     console.log("Inside verifyBBSProof  proof:", proof);
     console.log("Inside verifyBBSProof  publicKey:", publicKey);
     console.log("Inside verifyBBSProof  revealedMessages:", revealedMessages);
     console.log("Inside verifyBBSProof  nonce:", nonce);
   

     // Ensure revealedMessages are converted to Uint8Array
  const messagesToVerify = revealedMessages.map(msg => Uint8Array.from(Buffer.from(msg.value, "utf-8")));
  ...
  const proofToVerify = Uint8Array.from(Object.values(proof));  // Convert proof if it is not Uint8Array
  ...
  // console.log("Proof Verified: messagesToVerify : ", messagesToVerify); 
  // console.log("Proof Verified: messagesToVerify.slice(0, 2)", messagesToVerify.slice(0, 2)); 

  // const isProofVerified = await blsVerifyProof({
  //   proof: proofToVerify,
  //   publicKey: publicKey,
  //   messages: messagesToVerify.slice(0, 2),  // Verify only the first two claims (index 0 and 1)
  //   nonce: nonceToVerify,  // The same nonce used in proof creation
  //   revealed: [0, 1],  // Reveal only the first two claims (index 0 and 1)
  // });
